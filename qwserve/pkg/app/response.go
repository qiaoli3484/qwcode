...
func (r Gin) Response(code int, msg string, data interface{}) {
	//允许所有跨域请求
	r.Writer.Header().Set("Access-Control-Allow-Origin", "*")
	//允许跨域POST
	r.Writer.Header().Set("Access-Control-Request-Method", "*")
	r.JSON(200, gin.H{
		"code": code,
		"msg":  msg,
		"data": data,
...
func (r Gin) ResponseErr(code int, msg string) {
	r.String(code, "%s", msg)
}

func (r Gin) ResponseTotal(code int, msg string, data interface{}, total interface{}) {
	r.JSON(200, gin.H{
		"code":  code,
		"msg":   msg,
		"data":  data,
...
/*func (r Gin) ResponseByte(code int, msg string, data []byte) {
	str := `{"code":` + strconv.Itoa(code) + `,"msg":"` + msg + `","data":`

	r.Writer.Header().Set("Content-Type", "application/json")
	b := append(util.Str2bytes(str), data...)
	b = append(b, []byte("}")...)
	r.Writer.Write(b)
}*/

//Data too long for column 'FAvkzwb' at row 1
//数据太长
